...
function get($db_name, $table_name, $where_params)
{
	global $dbh;
	...
	{
		$where_text = "WHERE ".convert_params_into_text($where_params, TRUE);
	}
	$sql = "SELECT * FROM {$db_name}.{$table_name} $where_text";
	print("sql = $sql");
...
function get_list($db_name, $table_name, $where_params=array())
{
	global $dbh;
	if(empty($where_params))
	{
...
function insert($db_name, $table_name, $set_params)
{
	global $dbh;
	$insert_column_text = '';
	$insert_value_text = '';
	...
	foreach ($set_params as $key => $value)
	{
		...
		$insert_value_text .= ($value=='') ? 'NULL' : $value;
		$is_first_key = FALSE;
	}
	$sql = "INSERT INTO {$db_name}.{$table_name} ($insert_column_text) VALUES ($insert_value_text)";
	print("sql = $sql");
	$stmt = $dbh->prepare($sql);
	$stmt->execute(array());
}
